...
Для валидации токена необходимо чтобы адресат запроса в отправителе совпадал с адресом получателя, для этого добавлено в `docker-compose.yaml`:
...
Если токен есть, будем валидировать запросом на /token/introspect эндпоинт в keyckoak. Если токен невалиден, возвращаем код `Unauthorized`.
